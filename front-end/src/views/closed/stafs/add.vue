    ...
    <div v-if="visible" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div class="bg-white w-full max-w-lg rounded-xl shadow-xl flex flex-col max-h-[90vh]">
        ...
        <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <h2 class="text-base font-semibold text-gray-800">Add Staff</h2>
        ...
        <div class="flex justify-end gap-3 px-5 py-4 border-t border-gray-100 bg-gray-50 rounded-b-xl">
          ...
          <button form="staffForm" type="submit" :disabled="loading" class="btn-primary">
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        property_zone: "", owner_id: localStorage.getItem("userId"), is_staff: true,
      },
    };
  },
  async mounted() {
  ...
  methods: {
    async submitForm() {
      this.loading = true;
      try {
        await this.$apiPost("/create_staff", { ...this.form });
        this.$root.$refs.toast.showToast("Staff registered successfully", "success");
        this.$emit("success");
        this.$emit("close");
      } catch (e) {
        this.$root.$refs.toast.showToast(e.message || "Failed to register", "error");
