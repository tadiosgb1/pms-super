...
<template>
  <div class="min-h-screen bg-slate-50 p-3 md:p-4">
    <!-- Loading -->
    <Loading :visible="loading" message="Loading Tenant Properties..." />
    ...
    <div class="max-w-7xl mx-auto bg-white rounded-lg overflow-hidden border border-slate-200 shadow-sm">
      <div class="bg-gradient-to-r from-dprimary to-primary px-6 py-4 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h1 class="text-xl font-black text-white tracking-tight uppercase">Tenant Properties</h1>
            <p class="text-white/70 text-xs uppercase tracking-widest font-semibold">Property Sales Management</p>
          </div>
        </div>
      ...
      <div class="p-4 md:p-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
          <div class="relative w-full md:w-1/3">
            ...
            <input v-model="searchQuery" @input="fetchItems(1)" type="text" placeholder="Search properties..."
            ...
            <table class="min-w-full text-sm divide-y divide-slate-100">
              <thead class="bg-gradient-to-r from-dprimary to-primary">
                <tr>
                  ...
                  <th class="px-4 py-2.5 text-left text-xs font-black text-white uppercase tracking-wider">
                    <i class="fas fa-tag text-white/70 mr-2"></i>Listing Price
                  </th>
                  <th class="px-4 py-2.5 text-left text-xs font-black text-white uppercase tracking-wider">
                    <i class="fas fa-dollar-sign text-white/70 mr-2"></i>Selling Price
                  </th>
                  <th class="px-4 py-2.5 text-left text-xs font-black text-white uppercase tracking-wider">
                    <i class="fas fa-receipt text-white/70 mr-2"></i>Fee
                  </th>
                  <th class="px-4 py-2.5 text-left text-xs font-black text-white uppercase tracking-wider">
                    <i class="fas fa-home text-white/70 mr-2"></i>Property
                  </th>
                  <th class="px-4 py-2.5 text-left text-xs font-black text-white uppercase tracking-wider">
                    <i class="fas fa-user-tie text-white/70 mr-2"></i>Broker
                  </th>
                  <th class="px-4 py-2.5 text-left text-xs font-black text-white uppercase tracking-wider">
                    <i class="fas fa-user text-white/70 mr-2"></i>Buyer
                  </th>
                  <th class="px-4 py-2.5 text-left text-xs font-black text-white uppercase tracking-wider">
                    <i class="fas fa-check-circle text-white/70 mr-2"></i>Status
                  </th>
              ...
              <tbody class="bg-white divide-y divide-slate-100">
                <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-slate-50 transition-colors group">
                  ...
                  <td class="px-4 py-2 whitespace-nowrap text-xs font-semibold text-slate-800 group-hover:text-primary transition-colors">{{ item.listing_price }}</td>
                  <td class="px-4 py-2 whitespace-nowrap text-xs text-slate-600">{{ item.selling_price }}</td>
                  <td class="px-4 py-2 whitespace-nowrap text-xs text-slate-600">{{ item.posting_fee }}</td>
                  <td class="px-4 py-2 whitespace-nowrap text-xs text-slate-600 truncate max-w-xs">{{ item.property?.name }}</td>
                  <td class="px-4 py-2 whitespace-nowrap text-xs text-slate-600 truncate max-w-xs">{{ item.broker?.first_name }}</td>
                  <td class="px-4 py-2 whitespace-nowrap text-xs text-slate-600 truncate max-w-xs">{{ item.buyer?.first_name }}</td>
                  ...
                  <td class="px-4 py-2 whitespace-nowrap text-center">
                    <div class="flex justify-center gap-1">
                      ...
                      <button @click="approveSale(item.id)" class="w-7 h-7 flex items-center justify-center rounded-lg bg-green-50 text-green-600 hover:bg-green-600 hover:text-white transition-all shadow-sm" title="Approve">
                ...
                <tr v-if="items.length === 0">
                  <td colspan="8" class="text-center py-8 text-slate-400">
                    <div class="flex flex-col items-center">
                      ...
                      <p class="text-xs font-medium">No properties found</p>
                    </div>
                  </td>
                </tr>
              </tbody>
        ...
        <div class="md:hidden space-y-3">
          <div v-for="(item, index) in items" :key="item.id" class="bg-white border-l-4 border-l-primary rounded-lg shadow-sm p-4 border border-slate-200">
            <div class="flex justify-between items-start mb-3">
              <h2 class="font-bold text-slate-800 text-sm">Property #{{ index + 1 }}</h2>
            ...
            <div class="space-y-2 text-xs text-slate-700">
              <div>
                <span class="font-semibold text-slate-600">Listing Price:</span>
                <p class="text-slate-800">{{ item.listing_price }}</p>
              </div>
              <div>
                <span class="font-semibold text-slate-600">Selling Price:</span>
                <p class="text-slate-800">{{ item.selling_price }}</p>
              </div>
              <div>
                <span class="font-semibold text-slate-600">Fee:</span>
                <p class="text-slate-800">{{ item.posting_fee }}</p>
              </div>
              <div>
                <span class="font-semibold text-slate-600">Property:</span>
                <p class="text-slate-800">{{ item.property?.name }}</p>
              </div>
              <div>
                <span class="font-semibold text-slate-600">Broker:</span>
                <p class="text-slate-800">{{ item.broker?.first_name }}</p>
              </div>
              <div>
                <span class="font-semibold text-slate-600">Buyer:</span>
                <p class="text-slate-800">{{ item.buyer?.first_name }}</p>
              </div>
              <div>
                <span class="font-semibold text-slate-600">Status:</span>
                <span :class="[
                  'px-2 py-1 rounded-md text-[9px] font-black uppercase inline-block mt-1',
                  item.status === 'approved' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
                ]">
                  {{ item.status }}
                </span>
          ...
          <p v-if="items.length === 0" class="text-center text-slate-400 py-8 text-xs font-medium">No properties found.</p>
        </div>
    ...
    <!-- Add/Edit Modal -->
    <AddTenatProperty v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <EditTenatProperty v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>

    <!-- Delete Confirmation Modal -->
    <delete-confirm-modal 
      :visible="deleteModalVisible"
      title="Delete Property"
      message="Are you sure you want to delete this tenant property?"
...
<script>
import AddTenatProperty from "./AddTenatProperty.vue";
import EditTenatProperty from "./EditTenatProperty.vue";
import Loading from "@/components/Loading.vue";
import DeleteConfirmModal from "@/components/DeleteConfirmModal.vue";

export default {
  components: { AddTenatProperty, EditTenatProperty, Loading, DeleteConfirmModal },
  ...
  methods: {
    async approveSale(id){
     ...
     const res= await this.$apiPatch("/update_tenant_property_sale",id ,payload);
     //console.log("res",res);
    ...
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/get_tenant_property_sales', params);

        console.log("response tenant/broker property ",response);
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'TenatProperty-detail', params: { id } });
    },
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/delete_tenant_property_sale', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('TenatProperty deleted successfully', 'success');
      }
