...
<template>
  <div class="min-h-screen bg-slate-50 p-3 md:p-4">
    ...
    <div class="max-w-7xl mx-auto bg-white rounded-lg overflow-hidden border border-slate-200 shadow-sm">
      <div class="bg-gradient-to-r from-dprimary to-primary px-6 py-4 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h1 class="text-xl font-black text-white tracking-tight uppercase">System Configurations</h1>
            <p class="text-white/70 text-xs uppercase tracking-widest font-semibold">Application Settings</p>
          </div>
        </div>
      ...
      <div class="p-4 md:p-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
          <div class="relative w-full md:w-1/3">
            ...
            <input v-model="searchQuery" @input="fetchItems(1)" type="text" placeholder="Search configurations..."
            ...
            <table class="min-w-full text-sm divide-y divide-slate-100">
              <thead class="bg-gradient-to-r from-dprimary to-primary">
                <tr>
                  ...
                  <th class="px-4 py-2.5 text-left text-xs font-black text-white uppercase tracking-wider">
                    <i class="fas fa-key text-white/70 mr-2"></i>Key
                  </th>
                  <th class="px-4 py-2.5 text-left text-xs font-black text-white uppercase tracking-wider">
                    <i class="fas fa-sliders-h text-white/70 mr-2"></i>Value
                  </th>
              ...
              <tbody class="bg-white divide-y divide-slate-100">
                <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-slate-50 transition-colors group">
                  ...
                  <td class="px-4 py-2 whitespace-nowrap text-xs font-semibold text-slate-800 group-hover:text-primary transition-colors">{{ item.key }}</td>
                  <td class="px-4 py-2 whitespace-nowrap text-xs text-slate-600 truncate max-w-xs">{{ item.value }}</td>
                ...
                <tr v-if="items.length === 0">
                  <td colspan="4" class="text-center py-8 text-slate-400">
                    <div class="flex flex-col items-center">
                      ...
                      <p class="text-xs font-medium">No configurations found</p>
                    </div>
                  </td>
                </tr>
              </tbody>
        ...
        <div class="md:hidden space-y-3">
          <div v-for="(item, index) in items" :key="item.id" class="bg-white border-l-4 border-l-primary rounded-lg shadow-sm p-4 border border-slate-200">
            <div class="flex justify-between items-start mb-3">
              <h2 class="font-bold text-slate-800 text-sm">Configuration #{{ index + 1 }}</h2>
            ...
            <div class="space-y-2 text-xs text-slate-700">
              <div>
                <span class="font-semibold text-slate-600">Key:</span>
                <p class="text-slate-800">{{ item.key }}</p>
              </div>
              <div>
                <span class="font-semibold text-slate-600">Value:</span>
                <p class="text-slate-800">{{ item.value }}</p>
              </div>
            </div>
          </div>
          <p v-if="items.length === 0" class="text-center text-slate-400 py-8 text-xs font-medium">No configurations found.</p>
        </div>
    ...
    <!-- Add/Edit Modal -->
    <AddSystemConfigurations v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <EditSystemConfigurations v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
...
export default {
  components: { AddSystemConfigurations, EditSystemConfigurations, Loading, DeleteConfirmModal },
  ...
  methods: {
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'SystemConfigurations-detail', params: { id } });
    },
    ...
    async confirmDelete() {
      ...
      if(res) {
        this.$root.$refs.toast.showToast('SystemConfigurations deleted successfully', 'success');
      }
