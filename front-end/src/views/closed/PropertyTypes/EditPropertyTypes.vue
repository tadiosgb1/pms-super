    ...
    <div class="bg-white rounded-[2rem] shadow-2xl w-full max-w-2xl p-8 text-sm transform transition-all duration-300 scale-100 hover:scale-[1.01] max-h-[90vh] overflow-y-auto">
      ...
      <div class="flex justify-between items-center mb-6 pb-4 border-b-2 border-secondary/20 sticky top-0 bg-white">
        <div class="flex items-center gap-3">
          ...
          <h2 class="text-xl font-black text-slate-800 tracking-tight uppercase">Edit Property Type</h2>
        </div>
      ...
      <form @submit.prevent="submitForm" class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="group">
            ...
            <input 
              ...
              type="text" 
              required 
              class="border-2 border-slate-200 rounded-xl px-4 py-2.5 text-sm w-full focus:outline-none focus:ring-2 focus:ring-secondary/50 focus:border-secondary transition-all" 
          ...
          <div class="group">
            ...
            <textarea 
              ...
              required 
              rows="2"
              class="border-2 border-slate-200 rounded-xl px-4 py-2.5 text-sm w-full focus:outline-none focus:ring-2 focus:ring-secondary/50 focus:border-secondary transition-all resize-none" 
        ...
        <div class="flex justify-end gap-3 pt-4 sticky bottom-0 bg-white border-t-2 border-slate-100 -mx-8 px-8 py-4">
          <button 
            type="button" 
            @click="$emit('close')" 
            class="px-6 py-3 border-2 border-slate-200 rounded-xl font-bold text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-all text-xs uppercase tracking-wider"
          >
            Cancel
          </button>
          <button 
            type="submit" 
            class="px-6 py-3 bg-gradient-to-r from-secondary to-orange-600 text-white rounded-xl font-black shadow-lg shadow-secondary/30 hover:shadow-xl hover:shadow-secondary/40 transition-all text-xs uppercase tracking-wider hover:scale-105 active:scale-95"
          >
            <i class="fas fa-save mr-2"></i>Update Property Type
          </button>
        </div>
      </form>
    </div>
...
description: this.data?.description || ''
  ...
  methods: {
    async submitForm() {
      try {
        if ("Edit" === "Add") {
        const res= await this.$apiPost("/propertytypes", this.form);
        if(res){
           this.$root.$refs.toast.showToast('Added successfully', 'success');
         }

        } else {
         const res= await this.$apiPatch("/update_property_type",this.data.id ,this.form);
         if(res){
