    ...
    <div v-if="visible" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div class="bg-white w-full max-w-xl rounded-xl shadow-xl flex flex-col max-h-[90vh]">
        ...
        <div class="flex justify-end gap-3 px-5 py-4 border-t border-gray-100 bg-gray-50 rounded-b-xl">
          ...
          <button form="addSuperStaffForm" type="submit" class="btn-primary">Save</button>
        </div>
  ...
  methods: {
    async submitForm() {
      try {
        ...
        this.$root.$refs.toast.showToast("Super Staff created successfully", "success");
        this.$emit("success");
        this.$emit("close");
      } catch (error) {
        this.$root.$refs.toast.showToast(error.message || "Failed to create", "error");
      }
    },
  },
};
