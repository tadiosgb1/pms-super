<template>
  <Teleport to="body">
    <div v-if="visible" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div class="bg-white w-full max-w-xl rounded-xl shadow-xl flex flex-col max-h-[90vh]">
        ...
        <div class="overflow-y-auto p-5 space-y-4">
          <form id="updateGroupForm" @submit.prevent="confirmUpdate">
        ...
        <div class="flex justify-end gap-3 px-5 py-4 border-t border-gray-100 bg-gray-50 rounded-b-xl">
          ...
          <button form="updateGroupForm" type="submit" class="btn-primary">Save Changes</button>
        </div>
    ...
    <ConfirmModal :visible="showConfirm"
      title="Save Changes"
      :message="`Update permissions for '${form.name}'?`"
...
export default {
  ...
  props: { visible: Boolean, groupData: Object },
  data() {
    return {
  ...
  watch: {
    groupData: {
      immediate: true,
      handler(v) { if (v) { this.form = { id: v.id, name: v.name, permissions: [...v.permissions] }; } },
    },
  },
  ...
  methods: {
    ...
    confirmUpdate() { this.showConfirm = true; },
    async submitForm() {
      this.showConfirm = false;
      try {
        await this.$apiPost("/set_group_permissions", { group_id: this.form.id, permissions: this.form.permissions });
