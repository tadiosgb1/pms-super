    ...
    <div class="bg-white rounded-[2rem] shadow-2xl w-full max-w-2xl p-8 text-sm transform transition-all duration-300 scale-100 hover:scale-[1.01] max-h-[90vh] overflow-y-auto">
      ...
      <div class="flex justify-between items-center mb-6 pb-4 border-b-2 border-primary/20 sticky top-0 bg-white">
        <div class="flex items-center gap-3">
          ...
          <h2 class="text-xl font-black text-slate-800 tracking-tight uppercase">Add Category</h2>
        </div>
      ...
      <form @submit.prevent="submitForm" class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="group">
            ...
            <input v-model="form.name" type="text" required class="border-2 border-slate-200 rounded-xl px-4 py-2.5 text-sm w-full focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all" placeholder="Enter category name" />
          </div>
          ...
          <div class="group">
            ...
            <input v-model="form.description" type="text" required class="border-2 border-slate-200 rounded-xl px-4 py-2.5 text-sm w-full focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all" placeholder="Enter description" />
          </div>
        </div>
