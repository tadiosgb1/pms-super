...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800 relative">
    <!-- Loading -->
    <Loading :visible="loading" message="Loading tenantPropertySaleRequest..." />

    <!-- Page Header -->
    <div class="flex items-center justify-between mb-6 border-b pb-4 border-gray-200">
      <h1 class="text-lg font-bold text-gray-800">TenantPropertySaleRequest</h1>
      <button @click="openAddModal" class="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg font-medium shadow-md flex items-center space-x-1 text-sm">
        ...
        <span>Add TenantPropertySaleRequest</span>
      </button>
    </div>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-100 text-gray-700 uppercase text-xs font-semibold">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Message</th><th class="px-6 py-3 text-left">Requester</th><th class="px-6 py-3 text-left">Broker_property_sale</th><th class="px-6 py-3 text-left">Status</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-green-50 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap">{{ item.message }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.requester }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.broker_property_sale }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.status }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-white border border-gray-200 rounded-xl shadow p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-gray-800">TenantPropertySaleRequest #{{ index + 1 }}</h2>
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-700">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Broker_property_sale:</span>
              {{ item.broker_property_sale }}
            </div>
            <div class="col-span-2">
    ...
    <!-- Add/Edit Modal -->
    <AddTenantPropertySaleRequest v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <EditTenantPropertySaleRequest v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>

    <!-- Delete Confirmation Modal -->
    <delete-confirm-modal 
      :visible="deleteModalVisible"
      title="Delete TenantPropertySaleRequest"
      message="Are you sure you want to delete this tenantPropertySaleRequest?"
...
<script>
import AddTenantPropertySaleRequest from "./AddTenantPropertySaleRequest.vue";
import EditTenantPropertySaleRequest from "./EditTenantPropertySaleRequest.vue";
import Loading from "@/components/Loading.vue";
import DeleteConfirmModal from "@/components/DeleteConfirmModal.vue";

export default {
  components: { AddTenantPropertySaleRequest, EditTenantPropertySaleRequest, Loading, DeleteConfirmModal },
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/get_tenant_property_sale_requests', params);
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'TenantPropertySaleRequest-detail', params: { id } });
    },
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/tenantpropertysalerequest', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('TenantPropertySaleRequest deleted successfully', 'success');
      }
