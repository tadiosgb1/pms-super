    ...
    <div class="bg-white rounded-[2rem] shadow-2xl w-full max-w-2xl p-8 text-sm transform transition-all duration-300 scale-100 hover:scale-[1.01] max-h-[90vh] overflow-y-auto">
      ...
      <div class="flex justify-between items-center mb-6 pb-4 border-b-2 border-secondary/20 sticky top-0 bg-white">
        <div class="flex items-center gap-3">
          ...
          <h2 class="text-xl font-black text-slate-800 tracking-tight uppercase">Edit Tenant Property Sale Request</h2>
        </div>
      ...
      <form @submit.prevent="submitForm" class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="group">
            ...
            <input v-model="form.message" type="text" required class="border-2 border-slate-200 rounded-xl px-4 py-2.5 text-sm w-full focus:outline-none focus:ring-2 focus:ring-secondary/50 focus:border-secondary transition-all" placeholder="Enter message" />
          </div>

          <div class="group">
            <label class="block mb-2 text-xs font-black text-slate-700 uppercase tracking-wider">
              <i class="fas fa-user text-secondary mr-2"></i>Requester
            </label>
            <input v-model="form.requester" type="text" required class="border-2 border-slate-200 rounded-xl px-4 py-2.5 text-sm w-full focus:outline-none focus:ring-2 focus:ring-secondary/50 focus:border-secondary transition-all" placeholder="Enter requester" />
          </div>

          <div class="group">
            <label class="block mb-2 text-xs font-black text-slate-700 uppercase tracking-wider">
              <i class="fas fa-home text-secondary mr-2"></i>Broker Property Sale
            </label>
            <input v-model="form.broker_property_sale" type="text" required class="border-2 border-slate-200 rounded-xl px-4 py-2.5 text-sm w-full focus:outline-none focus:ring-2 focus:ring-secondary/50 focus:border-secondary transition-all" placeholder="Enter broker property sale" />
          </div>

          <div class="group">
            <label class="block mb-2 text-xs font-black text-slate-700 uppercase tracking-wider">
              <i class="fas fa-info-circle text-secondary mr-2"></i>Status
            </label>
            <input v-model="form.status" type="text" required class="border-2 border-slate-200 rounded-xl px-4 py-2.5 text-sm w-full focus:outline-none focus:ring-2 focus:ring-secondary/50 focus:border-secondary transition-all" placeholder="Enter status" />
          </div>
        </div>
        ...
        <div class="flex justify-end gap-3 pt-4 sticky bottom-0 bg-white border-t-2 border-slate-100 -mx-8 px-8 py-4">
          ...
          <button type="submit" class="px-6 py-3 bg-gradient-to-r from-secondary to-orange-600 text-white rounded-xl font-black shadow-lg shadow-secondary/30 hover:shadow-xl hover:shadow-secondary/40 transition-all text-xs uppercase tracking-wider hover:scale-105 active:scale-95">
            <i class="fas fa-check mr-2"></i>Edit Request
          </button>
        </div>
      </form>
    </div>
...
status: this.data?.status || ''
  ...
  methods: {
    async submitForm() {
      try {
        if ("Edit" === "Add") {
        const res= await this.$apiPost("/tenantpropertysalerequest", this.form);
        if(res){
           this.$root.$refs.toast.showToast('Added successfully', 'success');
         }

        } else {
         const res= await this.$apiPut("/tenantpropertysalerequest",this.data.id ,this.form);
         if(res){
